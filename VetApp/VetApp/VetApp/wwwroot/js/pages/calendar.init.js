...
function GetAppointmentsByDay() {
    $.ajax({
        type: "GET",
        url: "../Appointment/GetAppointmentsByDay",
        dataType: "json",
        success: function (res) {
            ...
            Object.keys(res).forEach(function (key) {
                ...
                let date = new Date(res[key].dayDate);

                let dateConversion = date.getFullYear() + '-' + validarMes(date) + '-' + date.getDate();

                element = {
                    title: res[key].doctorName,
                    start: dateConversion,
                    end: "",
                    className: "bg-warning"
                }
                /*console.log(key, res[key]);*/
                //array.title = res[key].doctorName;
                //array.start = res[key].dayDate;;
                array.push(element);
            });
...
! function (l) {
    ...
    e.prototype.onEventClick = function (e) {
        this.$formEvent[0].reset(), this.$formEvent.removeClass("was-validated"), this.$newEventData = null, this.$btnDeleteEvent.show(), this.$modalTitle.text("Editar Cita"), this.$modal.show(), this.$selectedEvent = e.event, l("#event-title").val(this.$selectedEvent.title), l("#event-category").val(this.$selectedEvent.classNames[0])
    }, e.prototype.onSelect = function (e) {
        this.$formEvent[0].reset(), this.$formEvent.removeClass("was-validated"), this.$selectedEvent = null, this.$newEventData = e, this.$btnDeleteEvent.hide(), this.$modalTitle.text("Agendar nueva cita"), this.$modal.show(), this.$calendarObj.unselect()
...
}(window.jQuery), window.jQuery.CalendarApp.init();